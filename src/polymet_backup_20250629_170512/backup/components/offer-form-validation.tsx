...
export function useOfferFormValidation<T extends Record<string, any>>(
  ...
  const handleChange = (fieldName: keyof T, value: any) => {
    ...
    setValidationState((prevState) => {
      ...
      if (!prevState[fieldKey]) return prevState;
  ...
  const handleSubmit = async (
    ...
    if (isValid) {
      ...
      for (const field in validationState) {
        if (validationState[field] && validationState[field].error) {
  ...
  return {
    ...
    errors: Object.fromEntries(
      Object.entries(validationState).map(([key, field]) => [key, field?.error || null])
    ),
